/*
* Visual Studio oblige l'utilisation de fonctions "sécurisées" mais elles
* ne sont pas standards. On force quand même Visual Studio
*/
#ifdef _MSC_VER 
...
int playGame( SDL_Window* window, SDL_Surface* surfaceWindow ) {
    ...
    for ( int i = 0; i < N_PIKACHU; i++ ) {
        ...
        pikachus = Create( CharacterCreate( "images/pikachu.bmp", CoordCreate( 0, 0 ) ), pikachus );
    ...
    while ( isOpen ) {
        ...
        while ( !IsEmpty( temp ) ) {
            ...
            if ( CharacterGetCoord( character ).x == CharacterGetCoord( sacha ).x && CharacterGetCoord( character ).y == CharacterGetCoord( sacha ).y ) {
